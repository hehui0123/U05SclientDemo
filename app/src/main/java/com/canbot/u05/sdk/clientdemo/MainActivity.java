    ...
    private boolean handleResult(int msgType, String msgData) {
        switch (msgType) {
            case MsgType.RECEIVER_MSG_VOICE_RECOGNIZER_RESULT:
                //识别结果为&&0,最终说的话
                Log.e(TAG, "收到识别结果：" + msgData);
                if (msgData.contains("&&0")) {
                    int index = msgData.indexOf("&&0");
                    Log.e(TAG, "查询下标--------" + index);
                    //截取有效话语
                    final String data = msgData.substring(0, index);
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(MainActivity.this, data, LENGTH_LONG).show();
                        }
                    });
                    if (data.equals("今天天气怎么样")) {     //本地处理
            ...
            case MsgType.RECEIVER_MSG_WAKE_UP:   //头部唤醒信息
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        makeText(MainActivity.this, "收到头部唤醒", LENGTH_SHORT).show();
                    }
                });
            ...
            case MsgType.HEAD_NO_WIFI_INFO://头部没有wifi信息时的回调
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        makeText(MainActivity.this, "头部没有连接WIFI", LENGTH_SHORT).show();
                    }
                });
                Log.e(TAG, "头部没有WIFI");
                break;
                ...
                if (msgData.contains(":")) {
                    ...
                    if (datas.length > 1) {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                makeText(MainActivity.this, "头部WIFI,ssid是:" + ssid + "pwd是:" + pwd, LENGTH_SHORT).show();
                            }
                        });
                        //胸口连接网络
                        WifiUtils.getInstance().connect(ssid, pwd);
                    } else {
                        Log.e(TAG, "头部发送的 wifi 不正确");
                    }
                }
                return true;
//                        case MsgType.VOLUME:
...
//                                return true;
                ...
                switch (msgData) {
                    case "0":
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                makeText(MainActivity.this, "没有充电", LENGTH_SHORT).show();
                            }
                        });
                        break;
                    case "1":
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                makeText(MainActivity.this, "正在充电", LENGTH_SHORT).show();
                            }
                        });
                        break;
                    case "2":
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                makeText(MainActivity.this, "正在自动回冲", LENGTH_SHORT).show();
                            }
                        });
                        break;
                    default:
            ...
            case MsgType.SEND_CLIENT_VERSION: {//接受版本号
                if (TextUtils.isEmpty(msgData)) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            makeText(MainActivity.this, "查询超时", LENGTH_SHORT).show();
                        }
                    });
                } else {
                    ClientVersionInfo versionInfo = parseObject(msgData, ClientVersionInfo.class);
    ...
    private IMsgCallBack iMsgCallback = new IMsgCallBack.Stub() {
        ...
        public boolean interceptRobotAction(final String dataJson) {
            ...
            StringMsgBean stringMsgBean = parseObject(dataJson, StringMsgBean.class);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(MsgType.ACTION_CONNECT_ROBOT_SUCCESSFULLY)) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        makeText(getApplicationContext(), "三方联网成功", LENGTH_SHORT).show();
                    }
                });
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        makeText(getApplicationContext(), "断开三方联网成功", LENGTH_SHORT).show();
                    }
                });
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        makeText(getApplicationContext(), "开始三方联网", LENGTH_SHORT).show();
                    }
                });
